...
import { ForensicMetadata, ThreatIntelligence, SuspiciousIndicator, TimelineEvent, ChainOfCustodyEntry } from '../types';
 ...
 */
const analyzeSuspiciousIndicators = (packet: ParsedPacket): SuspiciousIndicator[] => {
  const indicators: SuspiciousIndicator[] = [];
  const rawData = packet.rawData.toLowerCase();
 ...
 */
const checkThreatIntelligence = (packet: ParsedPacket): ThreatIntelligence[] => {
 ...
 */
const createForensicMetadata = (packet: ParsedPacket, investigator?: string): ForensicMetadata => {
 ...
 */
const createTimelineEvent = (packet: ParsedPacket): TimelineEvent => {
