...
export class CaptureController {
    public static async startCapture(req: Request, res: Response): Promise<void> {
        const { interface: interfaceName, promiscuous, filter, sizeLimit } = req.body;
        ...
        try {
            const session = await CaptureService.startCapture(interfaceName, filter, sizeLimit ? Number(sizeLimit) : 0);
            res.status(201).json({
            ...
            } else if (error.message.toLowerCase().includes('syntax') || error.message.toLowerCase().includes('expression')) {
    ...
    public static getStats(req: Request, res: Response): void {
        ...
        if (!interfaceName || typeof interfaceName !== 'string') {
            res.status(400).json({ error: 'Interface name is required (query param: interface)' });
            return;
        }
