...
export class YaraService {
    ...
    private readonly RULES_FILE = path.join(process.cwd(), 'data', 'yara_rules.json');
    ...
    private loadStoredRules() {
        try {
            if (fs.existsSync(this.RULES_FILE)) {
                this.ruleSources = JSON.parse(fs.readFileSync(this.RULES_FILE, 'utf-8'));
            } else {
                ...
                this.ruleSources = [
                    {
                        ...
                        source: 'rule SQLInjection_Classic { strings: $a = "OR \'1\'=\'1" condition: $a }'
                    },
                    {
                        name: 'SQLInjection_Union',
                        source: 'rule SQLInjection_Union { strings: $a = "UNION SELECT" nocase condition: $a }'
                    },
                    {
                        name: 'XSS_Attack',
                        source: 'rule XSS_Attack { strings: $a = "<script>" nocase condition: $a }'
                    },
                    {
                        name: 'Suspicious_JSON_Cmd',
                        source: 'rule Suspicious_JSON_Cmd { strings: $a = "\\"cmd\\": \\"exec\\"" condition: $a }'
                    },
                    {
                        name: 'Malicious_Site',
                        source: 'rule Malicious_Site { strings: $a = "Host: malicious-site.com" nocase condition: $a }'
                    },
                    {
                        name: 'Command_Injection_Eval',
                        source: 'rule Command_Injection_Eval { strings: $a = "eval(document.cookie)" condition: $a }'
                    },
                    {
                        name: 'Test_Rule_HTTP',
                        source: 'rule Test_Rule_HTTP { strings: $a = "HTTP" condition: $a }'
                    }
                ];
                this.saveRules();
            }
    ...
    private saveRules() {
        try {
            fs.writeFileSync(this.RULES_FILE, JSON.stringify(this.ruleSources, null, 2));
        } catch (e) { }
    }
        ...
        try {
            ...
            if (result.matchedRules) {
                for (let i = 0; i < result.matchedRules.size(); i++) {
                    ...
                    const resolvedMatches: { offset: number; length: number; data?: string }[] = [];
