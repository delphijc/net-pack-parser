...
export class ElasticService {
    ...
    private async ensureIndices(): Promise<void> {
        const exists = await this.client.indices.exists({ index: this.PACKET_INDEX });
        const settings = {
            index: {
        ...
        try {
            ...
            if (response.errors) {
                ...
                if (errorItems.length > 0) {
                    console.error('First error sample:', JSON.stringify(errorItems[0]?.error, null, 2));
                }
            }
        } catch (error) {
    ...
    public async searchPackets(sessionId: string, size: number = 100, from: number = 0): Promise<any> {
