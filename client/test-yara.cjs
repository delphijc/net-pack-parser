...
libyara().then(Module => {
    ...
    try {
        ...
        if (Module.run) {
            ...
            // Try as functions
            try { console.log('matched_rules():', result.matched_rules && result.matched_rules()); } catch (e) { console.log('matched_rules() failed'); }
            try { console.log('get_matched_rules():', result.get_matched_rules && result.get_matched_rules()); } catch (e) { console.log('get_matched_rules() failed'); }
            ...
            console.log('Prototype Property Names:', Object.getOwnPropertyNames(Object.getPrototypeOf(result)));
            ...
            if (result.matchedRules) {
                ...
                for (let i = 0; i < result.matchedRules.size(); i++) {
                    ...
                    console.log('Match keys:', Object.getOwnPropertyNames(Object.getPrototypeOf(match)));

                    if (match.resolved_matches) { // Might be resolvedMatches
                        console.log('  Resolved Matches (snake):', match.resolved_matches.size());
                    }
                    if (match.resolvedMatches) {
                        console.log('  Resolved Matches (camel):', match.resolvedMatches.size());
                        for (let j = 0; j < match.resolvedMatches.size(); j++) {
                            const rm = match.resolvedMatches.get(j);
                            console.log(`    Offset: ${rm.location}, Length: ${rm.match_length}, Data: ${rm.data}`);
                        }
                    }
                }
            }
            ...
            if (result.compileErrors) {
                ...
                for (let i = 0; i < result.compileErrors.size(); i++) {
                    ...
                    console.log(`Error ${i}: ${err.message} at line ${err.line_number}`);
                }
            }
