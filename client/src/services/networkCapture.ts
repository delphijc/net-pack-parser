 ...
 */
const createPerformanceEntryData = (
...
export const startNetworkCapture = async (
  callback: (packet: ParsedPacket) => void,
): Promise<void> => {
  ...
  try {
    ...
    const observer = new PerformanceObserver((list) => {
      ...
      for (const entry of entries) {
        if (entry.entryType === 'resource') {
          ...
          console.log(
            `Captured ${entry.entryType} performance entry:`,
            entry.name,
          );
        }
      }
    ...
    try {
      ...
      console.warn(
        'Some performance entry types not supported, using fallback:',
        error,
      );
