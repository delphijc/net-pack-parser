...
const parsePcapData = async (data: ArrayBuffer): Promise<ParsedPacket[]> => {
    ...
    try {
        ...
        for (const packet of decodedPackets) {
            ...
            const detectedFiles = fileExtractor.detectFileReferences(parsedPacket, new Uint8Array(rawPacketDataBuffer));
...
const parseStringData = (data: string): ParsedPacket => {
    try {
        ...
        parsedPacket.detectedProtocols = detectProtocols(parsedPacket, new Uint8Array(parsedPacket.rawData));
