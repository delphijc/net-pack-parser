...
class ChainOfCustodyDb {
    ...
    public async addFileChainOfCustodyEvent(event: FileChainOfCustodyEvent): Promise<void> {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                request.onerror = (event) => {
                    console.error('Error adding file chain of custody event:', (event.target as IDBRequest).error);
    ...
    public async getAllFileChainOfCustodyEvents(): Promise<FileChainOfCustodyEvent[]> {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                request.onerror = (event) => {
                    console.error('Error getting all file chain of custody events:', (event.target as IDBRequest).error);
                    reject((event.target as IDBRequest).error);
                };
            } catch (error) {
                reject(error);
