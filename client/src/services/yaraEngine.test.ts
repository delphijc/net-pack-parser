...
class MockWorker {
    ...
    postMessage(data: any) {
        ...
        setTimeout(() => {
            if (this.onmessage) {
                if (type === 'compile') {
                    this.onmessage({ data: { type: 'compileResult', id, success: true, ruleCount: payload.length } } as MessageEvent);
                } else if (type === 'scan') {
                    ...
                    this.onmessage({ data: { type: 'scanResult', id, success: true, matches } } as MessageEvent);
                }
            }
        }, 10);
    }
