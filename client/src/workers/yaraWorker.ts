...
const initPromise = libyara().then((Module: any) => {
    yaraModule = Module;
...
self.onmessage = async (e: MessageEvent) => {
    ...
    try {
        ...
        if (type === 'compile') {
            ...
            self.postMessage({ type: 'compileResult', id, success: true, ruleCount: payload.length });
            ...
            for (let i = 0; i < payload.length; i += chunkSize) {
                payloadStr += String.fromCharCode.apply(null, payload.subarray(i, i + chunkSize) as any);
            }
