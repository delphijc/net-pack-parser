...
export function detectSensitiveData(packet: Packet): ThreatAlert[] {
    ...
    const foundSensitiveData: { type: string; value: string; redacted: string }[] = [];
    let detectionContexts: string[] = [];

    for (const target of scanTargets) {
        const targetMatches: { offset: number; length: number }[] = [];
        const targetSensitiveData: { type: string; value: string; redacted: string }[] = [];

        const checkPattern = (pattern: RegExp, type: string, validator?: (val: string) => boolean) => {
            pattern.lastIndex = 0;
            let match;
        ...
        checkPattern(sensitiveDataPatterns.creditCard, 'Credit Card Number', validateLuhn);
    ...
    if (foundSensitiveData.length > 0) {
        ...
        const isCritical = uniqueSensitiveData.some(item =>
            ['AWS Access Key', 'AWS Secret Key', 'GitHub Token', 'Google API Key', 'Stripe Key', 'Private Key'].includes(item.type)
        );

        // Construct description with REDACTED values
        const descriptions = uniqueSensitiveData.map(item => `${item.type}: ${item.redacted}`);
        ...
        const rawDataDump = uniqueSensitiveData.map(item => `${item.type}: ${item.value}`).join('\n');
