...
describe('detectXss', () => {
    ...
    it('should detect simple script tags in query string', () => {
        const rawData = 'GET /search?q=<script>alert(1)</script> HTTP/1.1\r\nHost: example.com\r\n\r\n';
    ...
    it('should detect javascript: URIs', () => {
        const rawData = 'GET /?url=javascript:alert(1) HTTP/1.1\r\nHost: example.com\r\n\r\n';
        const packet = createPacket(rawData);
        const threats = detectXss(packet);
    ...
    it('should detect HTML entity encoded XSS', () => {
        ...
        // If raw:
        const rawData = 'GET /?q=&lt;script&gt;alert(1)&lt;/script&gt; HTTP/1.1\r\nHost: example.com\r\n\r\n';
        const packet = createPacket(rawData);
        const threats = detectXss(packet);
    ...
    it('should detect polyglots', () => {
        const rawData = 'GET /?q=javascript:/*--></title></style></textarea></script>*/alert(1) HTTP/1.1\r\nHost: example.com\r\n\r\n';
        const packet = createPacket(rawData);
        const threats = detectXss(packet);
    ...
    it('should not detect benign traffic', () => {
        const rawData = 'GET /search?q=hello+world HTTP/1.1\r\nHost: example.com\r\n\r\n';
        const packet = createPacket(rawData);
        const threats = detectXss(packet);
