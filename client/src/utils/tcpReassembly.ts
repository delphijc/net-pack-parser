...
export const reassembleTcpStream = (
  ...
  const flowPackets = packets.filter(
    (p) =>
      (p.flowId === flowId || p.sessionId === flowId) && p.protocol === 'TCP',
  );
  ...
  // 3. Map to segments
  const segments: StreamSegment[] = flowPackets.map((p) => {
    ...
    return {
      ...
      direction:
        p.sourceIP === clientIp && p.sourcePort === clientPort
          ? 'client-to-server'
          : 'server-to-client',
