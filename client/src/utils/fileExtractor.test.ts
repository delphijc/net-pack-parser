  ...
  constructor(parts: BlobPart[] = [], options?: BlobPropertyBag) {
    ...
    parts.forEach((part) => {
      ...
      if (typeof part === 'string') {
        ...
        part &&
        typeof part === 'object' &&
        part.constructor.name === 'Uint8Array'
      ) {
...
describe('FileExtractor', () => {
  ...
  describe('FileExtractor', () => {
    ...
    it('should detect FTP STOR command and create FileReference', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          'FTP command detected: STOR for file: myfile.txt',
        ),
      );
    ...
    it('should detect FTP RETR command and create FileReference', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          'FTP command detected: RETR for file: anotherfile.zip',
        ),
      );
      consoleSpy.mockRestore();
    });
  ...
  describe('calculateSha256', () => {
    ...
    it('should calculate SHA-256 hash for a Blob correctly', async () => {
      ...
      const expectedHashBuffer = new Uint8Array([
        0x1f, 0x82, 0x6e, 0x01, 0x7a, 0x3e, 0x98, 0x70, 0x24, 0xd0, 0x01, 0x76,
        0x70, 0xb2, 0x1f, 0x6e, 0x50, 0x1d, 0x14, 0x05, 0x5b, 0x15, 0x1d, 0x93,
        0x22, 0x66, 0x93, 0xe1, 0x25, 0xb4, 0x66, 0x9f,
      ...
      expect(hash).toBe(
        '1f826e017a3e987024d0017670b21f6e501d14055b151d93226693e125b4669f',
      );
    });
    ...
    it('should calculate SHA-256 hash for an ArrayBuffer correctly', async () => {
      ...
      const expectedHashBuffer = new Uint8Array([
        0x10, 0x82, 0x88, 0x50, 0x92, 0xc1, 0xd4, 0x96, 0x11, 0x47, 0x05, 0x56,
        0x37, 0x16, 0x89, 0x4b, 0x3d, 0x2e, 0x48, 0x89, 0x7b, 0x8d, 0x55, 0x8d,
        0x2c, 0xcb, 0x0a, 0x5e, 0x25, 0x24, 0x3d, 0x00,
      ...
      expect(hash).toBe(
        '1082885092c1d496114705563716894b3d2e48897b8d558d2ccb0a5e25243d00',
      );
    });
  });
  ...
  describe('reassembleFile', () => {
    it('should reassemble a file from multiple packets in correct order and calculate hash', async () => {
      ...
      const expectedHashBuffer = new Uint8Array([
        0x56, 0x4f, 0x4e, 0xc6, 0x29, 0x60, 0x66, 0x1c, 0x40, 0x69, 0x9d, 0x77,
        0x31, 0x4e, 0x5a, 0x9b, 0x55, 0x42, 0xd0, 0x22, 0x05, 0x8a, 0x47, 0x76,
        0x8a, 0x3d, 0x75, 0x6e, 0xc4, 0x2b, 0x1f, 0x7f,
      ...
      expect(reassembledFile.sha256Hash).toBe(
        '564f4ec62960661c40699d77314e5a9b5542d022058a47768a3d756ec42b1f7f',
      );
    });
    ...
    it('should handle fileReference missing sourcePacketId', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        'FileReference missing sourcePacketId for reassembly.',
      );
    ...
    it('should correctly sort packets by timestamp before reassembly', async () => {
      ...
      expect(
        new TextDecoder().decode(
          await (reassembledFile.data as Blob).arrayBuffer(),
        ),
