...
describe('extractStringsFromBuffer', () => {
  ...
  it('should extract IPv4 addresses', () => {
    const payload = stringToArrayBuffer(
      'Payload with IP 192.168.1.1 and another 10.0.0.255',
    );
    ...
    expect(
      result.some(
        (s) =>
          s.value === '192.168.1.1' &&
          s.payloadOffset === 'Payload with IP '.length,
      ),
    ).toBe(true);
  });
  ...
  it('should extract IPv6 addresses', () => {
    const payload = stringToArrayBuffer(
      'IPv6 address 2001:0db8:85a3:0000:0000:8a2e:0370:7334 and ::1',
    );
    const result = extractStringsFromBuffer(
      payload,
      mockPacketId,
  ...
  it('should extract URLs', () => {
    const payload = stringToArrayBuffer(
      'Visit https://www.example.com and ftp://files.test.org/doc.pdf',
    );
    const result = extractStringsFromBuffer(
      payload,
      mockPacketId,
  ...
  it('should extract email addresses', () => {
    const payload = stringToArrayBuffer(
      'Contact test@example.com or user.name+tag@sub.domain.co.uk',
    );
    const result = extractStringsFromBuffer(
      payload,
      mockPacketId,
  ...
  it('should extract potential credentials', () => {
    const payload = stringToArrayBuffer(
      'username=admin password=secret token=xyz auth=bearer_token api_key=abc',
    );
    const result = extractStringsFromBuffer(
      payload,
      mockPacketId,
  ...
  it('should extract printable ASCII strings longer than 4 characters', () => {
    const payload = stringToArrayBuffer(
      'Hello World \x00 short abc long_string',
    );
    const result = extractStringsFromBuffer(
      payload,
      mockPacketId,
  ...
  it('should correctly calculate payloadOffset and length', () => {
    ...
    const url = result.find(
      (s) => s.type === 'URL' && s.value === 'http://example.com',
    );
  ...
  it('should not extract strings shorter than MIN_PRINTABLE_STRING_LENGTH (4)', () => {
    const payload = stringToArrayBuffer(
      'one\x00two\x00three\x00four\x00five\x00six',
    );
    const result = extractStringsFromBuffer(
      payload,
      mockPacketId,
  ...
  it('should not add duplicate entries for the same string found by multiple regexes if value and offset are same', () => {
    const payload = stringToArrayBuffer(
      'This is a test. test@example.com This is another test.',
    );
    const result = extractStringsFromBuffer(
      payload,
      mockPacketId,
