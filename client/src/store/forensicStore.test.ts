...
describe('useForensicStore', () => {
  ...
  it('should manage case metadata', () => {
    ...
    useForensicStore
      .getState()
      .updateCaseMetadata({ summary: 'Updated summary' });

    expect(useForensicStore.getState().caseMetadata!.summary).toBe(
      'Updated summary',
    );
    expect(useForensicStore.getState().caseMetadata!.investigator).toBe(
      'John Doe',
    );
    expect(useForensicStore.getState().caseMetadata!.organization).toBe(
      'ACME Corp',
    );
  });
});
