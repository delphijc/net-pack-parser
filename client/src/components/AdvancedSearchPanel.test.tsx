...
describe('AdvancedSearchPanel', () => {
  ...
  it('renders all search fields correctly', () => {
    render(<AdvancedSearchPanel onSearch={mockOnSearch} onClear={mockOnClear} />);
    ...
    expect(screen.getByRole('combobox', { name: /Protocol/i })).toBeInTheDocument();
  ...
  it('clears all fields when Clear All is clicked', async () => {
    ...
    // Enter some data
    await user.type(screen.getByLabelText(/Source IP/i), '10.0.0.1');
  ...
  it('saves a new search configuration', async () => {
    ...
    expect(savedSearchService.saveSearch).toHaveBeenCalledWith('Malicious IP', expect.objectContaining({
  ...
  it('loads a saved search', async () => {
    ...
    expect(savedSearchService.loadSearch).toHaveBeenCalledWith('My Saved Search');
  ...
  it('deletes a saved search', async () => {
    ...
    const dialog = await screen.findByRole('alertdialog');
    expect(dialog).toBeInTheDocument();
    expect(within(dialog).getByText(/Are you sure you want to delete "Search To Delete"/i)).toBeInTheDocument();

    // Click Delete in dialog
    const confirmDeleteButton = within(dialog).getByRole('button', { name: 'Delete' });
    await user.click(confirmDeleteButton);

    expect(savedSearchService.deleteSearch).toHaveBeenCalledWith('Search To Delete');
