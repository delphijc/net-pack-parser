...
describe('FileInfo', () => {
  ...
  it('should display file information correctly', () => {
    const mockOnVerifyIntegrity = vi.fn(() =>
      Promise.resolve({ sha256Match: true, md5Match: true }),
    );
    render(
      <FileInfo
        {...mockFileInfoProps}
        onVerifyIntegrity={mockOnVerifyIntegrity}
      />,
    );
    ...
    expect(
      screen.getByText(`${mockFileInfoProps.fileSize} bytes`),
    ).toBeInTheDocument();
  ...
  it('should display "Mismatch Detected!" when hashes do not match', async () => {
    const mockOnVerifyIntegrity = vi.fn(() =>
      Promise.resolve({ sha256Match: false, md5Match: true }),
    ); // Only SHA-256 mismatches
    render(
      <FileInfo
        {...mockFileInfoProps}
        onVerifyIntegrity={mockOnVerifyIntegrity}
