    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onCloseOperation()}>
      <DialogContent className="max-w-4xl h-[80vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Stream ID: {flowId} | Total payload: {streamData.totalPayloadSize}{' '}
            bytes
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex justify-between items-center py-2">
          <Tabs
            value={viewMode}
            onValueChange={(v) => setViewMode(v as 'ascii' | 'hex')}
          >
            <TabsList>
          ...
          <Button variant="outline" onClick={onCloseOperation}>
            Close
          </Button>
        </div>
        ...
        <ScrollArea className="flex-1 border rounded-md p-4 bg-background font-mono text-sm leading-relaxed whitespace-pre-wrap">
          {streamData.segments.length === 0 ? (
            <div className="text-muted-foreground text-center italic">
              No payload data found for this stream.
            </div>
          ) : (
