...
export const NavigationTimingView = () => {
    ...
    // Phases in order
    const phases = [
        { name: 'DNS', value: metrics.dns, color: 'bg-blue-500', icon: Cloud, label: 'DNS Lookup' },
        { name: 'TCP', value: metrics.tcp, color: 'bg-indigo-500', icon: Server, label: 'TCP Connection' },
        { name: 'Req', value: metrics.request, color: 'bg-purple-500', icon: Globe, label: 'Request' },
        { name: 'Res', value: metrics.response, color: 'bg-pink-500', icon: Database, label: 'Response' },
        { name: 'DOM', value: metrics.domProcessing, color: 'bg-teal-500', icon: Box, label: 'DOM Processing' },
        { name: 'Load', value: metrics.loadEvent, color: 'bg-green-500', icon: Loader, label: 'Load Event' },
    ];
            ...
            <CardHeader>
                <CardTitle className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        <Badge variant="outline">TTFB: {(metrics.ttfb ?? 0).toFixed(0)}ms</Badge>
                        <Badge variant="secondary">Total Load: {total.toFixed(0)}ms</Badge>
                    </div>
                </CardTitle>
                <CardDescription>Breakdown of page load performance phases</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2 text-sm">
                        {phases.map((phase) => (
                            <div key={phase.name} className="flex flex-col items-center p-2 rounded border bg-card/50">
                                ...
                                <div className={`h-1 w-full rounded-full mt-2 ${phase.color}`}></div>
                            </div>
                        ))}
                    </div>
                </div>
